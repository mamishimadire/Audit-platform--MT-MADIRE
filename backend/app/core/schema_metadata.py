...
Pure functions over the plain dicts SQLAlchemy's inspector returns, so the
same logic runs on the platform's direct connections and — as a deliberate
dependency-free copy in gateway/gateway/schema_metadata.py, which cannot import
from the backend — inside a client's Gateway. A backend test runs both on the
same inputs and fails if they ever disagree.
