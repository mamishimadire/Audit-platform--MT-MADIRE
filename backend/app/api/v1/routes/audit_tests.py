...
def list_all(
    ...
    described = describe_tests(db, audit_test_ids=[t.audit_test_id for t in tests])
    out = []
    for test in tests:
        control_ids, domain, required_tables = described.get(test.audit_test_id, ([], None, []))
        row = AuditTestOut.model_validate(test)
        out.append(row.model_copy(update={"control_ids": control_ids, "domain": domain, "required_tables": required_tables}))
    return out
