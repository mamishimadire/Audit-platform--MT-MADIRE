...
from gateway.connectors.oracle import OracleConnector
from gateway.connectors.postgres import PostgresConnector
from gateway.connectors.sap_hana import SapHanaConnector
...
def build_connector(source_type: str, config: ConnectorConfig):
    """
    Connector selection by source_type string: adding an engine means adding one class here, not touching
    the discovery or reporting logic (product spec Section 12/17: connector abstraction, never a single
    database type hard-coded into the application).
    """
    try:
