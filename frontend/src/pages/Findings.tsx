...
export function FindingsPage() {
  ...
  const visibleFindings = findings.filter((f) => {
    if (view === 'active') { if (f.status === 'closed') return false }
    else {
    ...
    if (!term) return true
    const domain = (f.control_code ? domainByCode.get(f.control_code) : undefined) ?? ''
    return (
      (f.control_code ?? '').toLowerCase().includes(term) ||
      (f.control_name ?? '').toLowerCase().includes(term) ||
      f.finding_title.toLowerCase().includes(term) ||
      (f.finding_description ?? '').toLowerCase().includes(term) ||
      domain.toLowerCase().includes(term)
    )
  })

  // Grouped by audit domain, then by control within it — same reasoning as
  // the Exceptions page, and the same shared helper.
  ...
  return (
    <div>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        <input
          placeholder="Search by control, category or title"
          value={search}
            ...
            {domainGroups.map((domainGroup) => (
              <Fragment key={domainGroup.domain}>
                <tr className="border-t border-line bg-ink/[0.04]">
                  <td colSpan={5} className="px-4 py-1.5 text-xs font-bold uppercase tracking-wide text-ink">
